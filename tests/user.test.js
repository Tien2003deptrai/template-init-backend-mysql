    ...
    await request(app)
      ...
      .expect(function (res) {
        if (!('error' in res.body) && !('msg' in res.body)) {
          ...
          res.body.msg !== 'Sign up successful.'
        ) {
    ...
    await request(app)
      ...
      .expect(function (res) {
        if ('error' in res.body) {
          ...
          !('first_name' in res.body) &&
          !('last_name' in res.body) &&
          !('email' in res.body) &&
          !('interests' in res.body)
        ) {
          throw new Error(
            'Response should contain first name, last name, email and interests.'
          );
        }
    ...
    await request(app)
      ...
      .expect(function (res) {
        if (res.body.error !== false) {
          ...
          !('first_name' in res.body.user) &&
          !('last_name' in res.body.user)
        ) {
